...
function detect_itinerary_date_column_by_try(mysqli $conn) {
  ...
  foreach ($candidates as $c) {
    ...
    try { $stmt = @$conn->prepare($sql); } catch (Throwable $e) { $stmt = false; }
    if ($stmt) { $stmt->close(); return $c; }
  }
  return ['col'=>'DayNumber','uses_day_number'=>true];
}
...
if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['__step'] ?? '')==='submit') {
  ...
  if (!$final) {
    ...
    $pFName = $f; $pLName = $l; $pEmail = $email; $pPhone = $phone; $pNIC = $nic;
    $pStart = $startDate; $pEnd = $endDate; $pPickup = $pickup; $pDrop = $drop;
    $pPeople = (int)$people; $pGuidePref = (int)($wantGuide ? 1 : 0);
    $pPrice = (int)round($totalPrice); $pPayMethod = $payMethod;
    $pPayStatus = ($payMethod==='Online' ? (!empty($_SESSION['paid_ok']) ? 'Paid' : 'Unpaid') : 'Unpaid');
    if ($payMethod==='Online' && empty($_SESSION['paid_ok'])) { $final = ["ok"=>false,"msg"=>"Please complete the online payment first."]; }
  }
  if (!$final) {
    $pDriverEarn = (int)round($driverCost); $pGuideEarn = (int)round($guideCost);
    $pUserId = (int)$uid; $pDriverId = (int)$driverId; $pGuideId = $wantGuide ? (int)$chosenGuideId : null; $pPackageId = null;
    ...
    if ($bp === false) {
      ...
      $bp->bind_param(
        $types,
        $pFName, $pLName, $pEmail, $pPhone, $pNIC,
        $pStart, $pEnd, $pPickup, $pDrop,
        $pPeople, $pGuidePref,
      ...
      if ($bp->execute()) {
        ...
        if (is_array($itArr)) {
          ...
          $ins = null; $picked = null;
          foreach ($candidates as $c) {
            $trySql = "INSERT INTO booking_destinations (Booking_ID, {$c['col']}, Destination) VALUES (?,?,?)";
            try { $ins = @$conn->prepare($trySql); } catch (Throwable $e) { $ins = false; }
            if ($ins) { $picked = $c; break; }
          }
          if ($ins && $picked) {
            ...
            if ($usesDayNumber) {
              foreach ($itArr as $d) {
                $dDate = isset($d['date']) ? (string)$d['date'] : null;
                if ($dDate && !isset($dayMap[$dDate])) { $dayMap[$dDate] = $i++; }
              }
            }
            foreach ($itArr as $day) {
              ...
              foreach ($dests as $dest) {
                ...
                if ($usesDayNumber) {
                  $dayNumberBind = (int)($dayMap[$dDate] ?? 0);
                  $destBind = $destName;
        ...
        $itRows = [];
        $sel = null;
        foreach (['Day_Date','Date','DayNumber'] as $col) {
          try { $try = @$conn->prepare("SELECT {$col} AS DCol, Destination FROM booking_destinations WHERE Booking_ID=? ORDER BY {$col}, Destination"); }
          catch (Throwable $e) { $try = false; }
          if ($try) { $sel = $try; break; }
        }
        if ($sel) {
          $bid = (int)$newId;
          ...
          $sel->execute();
          $rs = $sel->get_result();
          while ($r = $rs->fetch_assoc()) { $itRows[] = $r; }
          $sel->close();
        }
        ...
        $final = [
          ...
          "guide"=>$wantGuide, "guideId"=>$pGuideId,
          "totalKm"=>$totalKm, "driverCost"=>0.0, "guideCost"=>0.0, "totalPrice"=>$pPrice,
...
<style>
  ...
  .step{padding:.4rem .6rem;border-radius:999px;background:#f1f3f5;color:#495057}
  .step.active{background:#0d6efd;color:#fff}
  .step-pane{margin-top:1rem}
  .day-card{border:1px solid #e9ecef;border-radius:.75rem;padding:1rem;margin-bottom:.75rem}
  .dest-row{display:flex;gap:.5rem;margin-top:.5rem}
  ...
  .calc-box,.summary-box,.review-box{border:1px solid #e9ecef;border-radius:.75rem;padding:1rem}
  .it-row{display:flex;gap:1rem;margin:.3rem 0}
  .it-dests span{background:#f1f3f5;border-radius:999px;padding:.15rem .5rem;margin-right:.25rem}
  .btn-success{background-color:#16a34a;border-color:#16a34a}
  .phone-row{display:flex;gap:.5rem;align-items:end}
  .cc-wrap{min-width:220px}
  .cc-search{position:relative}
  .cc-search input{padding-right:2rem}
  .cc-search .cc-clear{position:absolute;right:.5rem;top:50%;transform:translateY(-50%);border:none;background:transparent}
</style>
</head>
<body>
...
<?php
$prefill = [
  'F_Name'=>'','L_Name'=>'','Email'=>'','Phone_Country'=>'+94 ','Phone_Local'=>'',
  'NIC_or_Paasport'=>'','Number_of_People'=>'1','Start_Date'=>'','Duration_Days'=>'1',
  'Pickup_Location'=>'','End_Location'=>'','Vehicle_Category'=>'','Want_Guide'=>'0','Guide_ID'=>'','Itinerary_JSON'=>'[]','Total_KM'=>'0'
];
          ...
          <div class="row g-3">
            ...
            <div class="col-md-6">
              ...
              <div class="phone-row">
                <div class="cc-wrap">
                  <div class="cc-search">
                    <input type="text" class="form-control" id="ccSearch" placeholder="Type country or code">
                    <button type="button" class="cc-clear" id="ccClear">×</button>
                  </div>
                  <select class="form-select mt-1" id="Phone_Country" name="Phone_Country">
                    <option value="<?= htmlspecialchars($prefill['Phone_Country']) ?>" selected>Sri Lanka (+94)</option>
                  </select>
                </div>
                <input type="text" name="Phone_Local" id="Phone_Local" class="form-control" required value="<?= htmlspecialchars($prefill['Phone_Local']) ?>">
              </div>
            </div>
            <div class="col-md-6">
...
function goStep(n){ steps.forEach(s=>s.classList.toggle('active', s.dataset.step==n)); panes.forEach(p=>p.classList.toggle('d-none', p.dataset.step!=n)); }
document.getElementById('toStep2')?.addEventListener('click', ()=>{
  ...
  if (!document.getElementById('Phone_Local').value.trim()) return;
...
document.getElementById('toStep3')?.addEventListener('click', ()=>{ goStep(3); buildVehicleOptions(); refreshGuides(); });
document.getElementById('backTo2')?.addEventListener('click', ()=>goStep(2));
document.getElementById('toStep4')?.addEventListener('click', async ()=>{ buildReview(); await calcDistance(); await updateQuote(); goStep(4); syncConfirmState(); });
...
guideList?.addEventListener('change', (e)=>{ if (e.target && e.target.name==='Guide_ID') guideIdHidden.value = e.target.value; });

document.getElementById('calcBtn')?.addEventListener('click', async ()=>{ await calcDistance(); await updateQuote(); });
...
async function updateQuote(){
  ...
  const res = await fetch(url.toString(), { headers: { 'X-Requested-With':'XMLHttpRequest' } });
  let data = {}; try { data = await res.json(); } catch(e){}
  if (data && data.ok){ revTotal.textContent = "USD " + Math.round(Number(data.total)).toFixed(0); }
}

document.getElementById('customForm')?.addEventListener('submit', ()=>{ if (!totalKmField.value) totalKmField.value = "0"; });
...
const ccSelect = document.getElementById('Phone_Country');
    ...
    data.forEach(c=>{
      ...
      if (root && suffixes.length){
        suffixes.forEach(suf=>{
          list.push({label: `${name} (${root}${suf})`, value: `${root}${suf} `, name});
        });
      }else if (root){
        list.push({label: `${name} (${root})`, value: `${root} `, name});
      }
    });
    list.sort((a,b)=>a.name.localeCompare(b.name));
    ccOptions = list;
    renderCcOptions('');
    const preset = ccSelect.querySelector('option')?.value || '+94 ';
    ccSelect.value = preset;
  }catch(e){}
}
function renderCcOptions(filter){
  ccSelect.innerHTML = '';
  const f = (filter||'').toLowerCase();
  const items = ccOptions.filter(o=>!f || o.label.toLowerCase().includes(f) || o.value.toLowerCase().includes(f));
  items.forEach(o=>{
...
ccSearch?.addEventListener('input', (e)=>renderCcOptions(e.target.value));
ccClear?.addEventListener('click', ()=>{ ccSearch.value=''; renderCcOptions(''); });
loadCountryCodes();
...
payNowBtn?.addEventListener('click', ()=>{
  ...
  ['F_Name','L_Name','Email','NIC_or_Paasport','Number_of_People','Start_Date','Duration_Days',
   'Pickup_Location','End_Location','Vehicle_Category','Guide_ID','Total_KM','Itinerary_JSON'].forEach(k=>{
  ...
  dst.querySelector('[name="Phone_Country"]').value = document.getElementById('Phone_Country').value || '+94 ';
  dst.querySelector('[name="Phone_Local"]').value = document.getElementById('Phone_Local').value || '';
